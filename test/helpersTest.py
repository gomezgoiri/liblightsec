...
class HelpersTest(unittest.TestCase):
    ...
    def test_sample_use_case(self):
        """
        Basic communication example between a user and a sensor (see the diagram in README.md).
        """
        id_user = "user1"
        ...
        kenc, kauth = sensor.create_keys(id_user, stuff["a"], stuff["init_time"], stuff["exp_time"],
                                         user.initial_counter)
        ...
        self.assertTrue(sensor.msg_is_authentic(deciphertext, mactext, id_user, stuff["a"], stuff["init_time"],
                                                user.initial_counter))
        ...
        # test that the sensor is able to understand the second (and subsequent) message(s) sent by the user
        second_message = "I am sending the second message"
        ciphertext2 = user.encrypt(second_message)
        deciphertext2 = sensor.decrypt(id_user, ciphertext2)
        self.assertSequenceEqual(deciphertext2, second_message)

        # test that the sensor is able to authenticate the second message
        mactext2 = user.mac(second_message)
        self.assertTrue(sensor.msg_is_authentic(deciphertext2, mactext2, id_user))
